...
OneMinCalculator.prototype.start = function(){
    ...
    setInterval(function(){
        self.doCalc();
    }, 10000);// self.calcInterval);
    console.log("OneMinCalculator#start");
};

OneMinCalculator.prototype.doCalc = function(){

    var queue = this.dataSource.findAll();
    for(var i in queue){
        if(i === undefined) continue;
        // search
        var bar = this.createBar(queue[i]);
...
OneMinCalculator.prototype.search = function(){
    
};

OneMinCalculator.prototype.createBar = function(data){
    var first = true;
    ...
    for(var i in data){
        ...
        currencyPair = data[i].currencyPair;
